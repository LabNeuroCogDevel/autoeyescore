...
preprocessEyd <- function(outputTable=NULL, xmax=261, minSpikeVel=30, adjFilter=rep(1/5,5), adjThresh=0.4, smoothFilter=c(0.1,0.2,0.4,0.2,0.1),
  opts=list(capped=T, removeCapped=T, spikes=T, blink=T, blinkID="pupil", removeAdj=T, smooth=T)) {
  ...
  if (opts$spikes) {
    spikes <- c(F, rollapply(xvel, 2, function(x) sign(prod(x)) == -1 && abs(min(x)) > minSpikeVel))
  } else {
  ...
  # record blinks
  if (opts$blink) {
    # option "pupil": blink if pupil diameter=0; option "all": blink if pupil diameter, horz and vert position are all 0 
  ...
  # interpolate points adjacent to removed samples
    # will uses 2, but then some extra code in case this may leave a stranded point
  ...
  if (opts$removeAdj) {
    adjacent <- naFill(filter(replacePoints, adjFilter) >= adjThresh) # after filter, need to replace NAs at end
  } else {
  ...
  xpos <- naFill(na.approx(zoo(xpos), na.rm=F)) # need "zoo" library for na.approx
  ...
  # write table
  preproc <- data.frame(raw, xpos, xvel, cappedX, capped, spikes, zeros, adjacent, replacePoints)
  if (!is.null(outputTable)) write.table(preproc, file=outputTable, row.names=F, quote=F)
...
getSaccades <- function(outputTable=NULL, minVel=4, slowVel=1, minSacLength=4, minSacGap=3, replacedThr=0.7,
  opts=list(merge=T)) {
  ...
  while (TRUE) {
    ...
    if (end - start >= minSacLength - 1 || replaced / (end - start + 1) <= replacedThr) {
...
# ==============================================================================
# variables: "xposTargetList", "expectedTrialCount", "trialTypes", "xdatList", "whichIndex", "targetDuration", "customTrialCode"
...
settingsList$AntiState <- function() {
  ...
  makeList(c("xposTargetList", "expectedTrialCount", "trialTypes", "xdatList", "whichIndex", "targetDuration", "fixCode", "customTrialCode"))
}
...
settingsList$MGSEncode <- function(){
  ...
  # fixation codes (should just be 250, but actually the fix blocks alternate between three codes
...
scoreTrial <- function(startInd, minOnsetDelay=4, preTargetFix=9, blinkSample=6, sacMinMag=8, sacHeld=6,
  opts=list(blinkDrop=F, magCheck=F, heldCheck=T)) {
  ...
  if (!exists("xposTarget")){
    xposInd <- switch(settings$whichIndex[type], first = startInd, after = startInd-1)
  ...
  within(scoring, {
    ...
    if (opts$magCheck && abs(preproc$xpos[end[ind]] - preproc$xpos[start[ind]]) < sacMinMag) {
      dropped[ind] <- T
    ...
    while (ind + add < length(start) && end[ind+add+1] < startInd + settings$targetDuration[type]) {
      add <- add + 1
      scored[ind+add] <- T
      ...
      locationAct <- preproc$xpos[end[ind+add]] # new saccade location
      sideAct <- sign(locationAct - xposCenter)
      ...
      if (opts$magCheck && abs(preproc$xpos[end[ind+add]] - preproc$xpos[start[ind+add]]) < sacMinMag) {
        dropped[ind+add] <- T
...
scoreRun <- function(outputTable=NULL, xposCenter=261/2, xposPadding=30,
  ...
  # if preprocessed data not present, stop
  check <- checkVars(c("preproc", "saccades", "runData", "settings")); if (!is.null(check)) stop(check)

  # need to switch scoreTrial environment so it can see variables in this function
  ...
  if (opts$fixCheck) {
    xposCenterFixAll <- mean(preproc$xpos[which(preproc$XDAT %in% settings$fixCode)], na.rm=T)
    if (xposCenterFixAll - xposCenter > xposPadding) stop(paste("mean of actual eye fixation across run (", xposCenterFixAll, ") differs from expected eye fixation (", xposCenter, ") by >", xposPadding, sep=""))
  }

  # run for each trial type and trial
  for (type in 1:length(settings$trialTypes)) for (t in 1:length(which(runData$trialType == type))) scoring <- scoreTrial(runData$startInd[runData$trialType == type][t])
...
getRunData <- function(outputTable=NULL,
  opts=list()) {

  # if necessary variables are not present, stop
  check <- checkVars(c("raw", "settings", "task", "filePrefix", "taskData")); if (!is.null(check)) stop(check)

  # short variable name to save space, since we use it a few times
  N <- settings$expectedTrialCount[type]
  ...
  # format runData so has trial number, trial type, time (s), (will add index to this after merging below)
  runData <- switch(task,
    MGSEncode = taskData[grep(r, taskData$run), ],
    AntiState = taskData[grep(s, taskData$script)), ]
  )
  # duplicating so we have for both trial types (already duplicated in antistate)
  if (task == "MGSEncode") {
  ...
  runData <- data.frame(runData, list2data(lapply(1:length(settings$trialTypes), function(type) {
    ...
    # if incorrect number of trials, stop -- no longer need to error out, will match to expected trials
    #if(length(startInd) != settings$expectedTrialCount[type]) stop(paste("number of ", settings$trialTypes[type], " trials detected from xdats (", length(startInd), ") do not match expectedTrialCount (", settings$expectedTrialCount[type], ")", sep=""))

    # index of start of run to get times from xdats
    if (type == 1) {
      runStartInd <- switch(task,
        MGSEncode = startInd[1] - 90, # starts 1.5s (90 samples) before first cue
        AntiState = startInd[1] - ms2asl(1000 * runData$time[1]) # variable time to first trial (will model as 4.5s block)
      )
    }
    ...
    if (diff != 0) {
      trialMatch <- sapply(1:length(xdatTime), function(i) which.min(abs(xdatTime[i] - runData$time)))
      if (diff < 0) {
        uniqueCount <- rle(trialMatch)$l
        excludeInd <- unlist(lapply(which(uniqueCount > 1), function(ind) {
          trial <- rle(trialMatch)$v[ind]
          xdatInd <- c(1, 1 + cumsum(uniqueCount)); xdatInd <- seq(xdatInd, xdatInd + uniqueCount[ind] - 1)
          xdatInd[-which.min(abs(xdatTime[xdatInd] - runData$time[which(runData$trial %in% trial)]))]
        }))
        ...
        interpolateTime <- predict(lm(xdatTime ~ matchingTrials), data.frame(matchingTrials = missingTrials))
        for (i in missingTrials) {
          xdatTime <- c(xdatTime[1:(i-1)], interpolateTime[i], xdatTime[i:length(xdatTime)])
          startInd <- c(startInd[1:(i-1)], runStartInd + ms2asl(1000 * interpolateTime[i]), startInd[i:length(startInd)])
        }
      }
    }
  ...
  # write table and return scored saccades
  if (!is.null(outputTable)) write.table(runData, file=outputTable, quote=F)
  runData
}
...
# ==============================================================================
# two options:
# 1) correct from fixation: using trial start times (from runData), get median xpos during fixation for each trial
# 2) correct from vgs accuracy: use accuracy for correct vgs trials
# in both cases, fit linear model (accuracy vs. time), remove high influence points, predict for all indices, return correction

correctCalibration <- function(correctionMethod, trialType, fixDuration=90, outputTable=NULL,
  opts=list()) {

  # if necessary variables are not present, stop
  check <- checkVars(c("runData")); if (!is.null(check)) stop(check)
  ...
  # 
  } else stop("correction methods value of", correctionMethod, "is invalid, must be either scored or fixation")

  # return correction
  predict(correctionModel, data.frame(xpos = 1:dim(raw)[1]))
}
...
summaryData <- function(outputTable=NULL) {
  ...
  # if preprocessed data not present, stop
  check <- checkVars(c("settings", "scored")); if (!is.null(check)) stop(check)
  ...
  summaryData <- list2data(with(scored, {
    ...
    #summaryData <- data.frame(type=character(), count=numeric(), correct=numeric(), incorrect=numeric(), corrected=numeric(), dropped=numeric(), droppedReason=character(), percCorrect=numeric(), latency=numeric(), accuracy=numeric(), accuracyMost=numeric(), stringsAsFactors=F)
      ...
      diff <- settings$expectedTrialCount[t] * max(listInd) - summaryData$count[t]
      if (diff > 0) summaryData <- within(summaryData, {
        count[t] <- count[t] + diff
        dropped[t] <- dropped[t] + diff
        droppedReason[t] <- paste(droppedReason[t], paste("no_saccades_left", diff, sep=":"), sep=",")
      })
...
taskList$MGSEncode <- function(path="~/Dropbox/COG", prefix="mgsRun", output="task_MGSEncode.txt", useOld=T){
  ...
  stimData <- data.frame(run=numeric(total), cueTime=numeric(total), cueLength=numeric(total), delayTime=numeric(total), delayLength=numeric(total), targetTime=numeric(total), targetPos=numeric(total))
  ...
  for (run in 1:runs) {
    ...
    for (t in 1:trialCount) {
      ...
      stimData[t + (run-1) * trialCount, ] <- data.frame(run, cueTime, cueLength, delayTime, delayLength, targetTime, targetPos)
    }
  }
...
taskList$AntiState <- function(path="~/Dropbox/COG", file="Anti_Mix_Design_Lists_FINAL_minusfix_fixblockrecord_FINMOD.xls", output="task_AntiState.txt", useOld=T){
  ...
  # data frame for stimulus data
  stimData <- data.frame(script=numeric(total), time=numeric(total), type=character(total), targetPos=numeric(total))
  ...
  for (s in 1:length(scripts)) {
    ...
    stimData[1:trialCount + (s-1) * trialCount, ] <- data.frame(
      ...
      cueTime = xls$Start.Tm[trialIndex-2], # cue starts 2 units (3s) before target 
      targetTime = xls$Start.Tm[trialIndex],
      type = if (length(grep("AV", scripts[s])) == 0) rep(types, each=trialCount/2) else rep(rev(types), each=trialCount/2),
...
checkVars <- function(vars) { for (v in vars) if (!exists(v)) return(paste(v, "not found")); NULL }

# concatenate data frames stored as elements in a list
list2data <- function(l) {
  d <- cbind(l[[1]], listInd=1)
  if (length(l) > 1) for (i in 2:length(l)) d <- rbind(d, cbind(l[[i]], listInd=i))
  d
}

# make a named list from a group of variables
makeList <- function(vars) { l <- list(); for (v in vars) l[[v]] <- get(v, envir=parent.frame()); l }

# convert asl units
asl2ms <- function(t) round(t * 1000 / 60) # time, 60Hz
ms2asl <- function(t) round(t * 60 / 1000) # time, 60Hz
asl2deg <- function(m) round(m / 14.5, 2) # magnitude, 14.5 is an approximation but so is the 3/6/9deg vs 106/106/101px
deg2asl <- function(m) round(m * 14.5, 0) # magnitude, 14.5 is an approximation but so is the 3/6/9deg vs 106/106/101px
