...
for (file in files){
  ...
  raw <- try(
    ...
    raw <- read.table(file.path(path, file), head=T),
    silent=T)
  if (class(raw) != "data.frame") { cat(date(), "\n", filePrefix, "raw", "\n", raw, "\n\n"); next } 

  # preprocess data
  preproc <- try(
    preprocessEyd(raw, outputTable=file.path(path, paste(filePrefix, "preproc.txt", sep="_"))),
    silent=T)
  if (class(preproc) != "data.frame") { cat(date(), "\n", filePrefix, "preproc", "\n", preproc, "\n\n"); next } 
  ...
  # score saccades
  scored <- try(
    scoreRun(task, preproc, saccades, outputTable=file.path(path, paste(filePrefix, "scored.txt", sep="_"))),
    silent=T)
  if (class(saccades) != "data.frame") { cat(date(), "\n", filePrefix, "scored", "\n", scored, "\n\n"); next
