...
inspect_trial <- function(dat, only_trial=1, trial_desc="",
                          ...
                          events=NULL, score_fnc=NULL, max_x=NULL) {
    ...
    this_score<-NULL
    if(!is.null(score_fnc)) {
       this_score <- score_fnc(dat) %>% filter(trial %in% only_trial)
    }
    ...
    p <- ggplot(t_raw) +
     ...
     if(!is.null(this_score)) p <- p +
        ggtitle(paste0(collapse=" ",'trial', paste00(only_trial), paste00(trial_desc), 'score:', paste00(this_score$score) ,'lat:', paste00(this_score$lat),"; " fname))
     return(p)
}
