...
## FUNCTIONS
# where are the files?
filebasedir <- '/Volumes/L/bea_res/Data/Tasks/Anti7TASL'
getFiles <- function(filesFrom=sprintf('%s/*/*/Raw/EyeData/txt/*.data.tsv',filebasedir)) {
 ...
 # /Volumes/L/bea_res/Data/Tasks/Anti7TASL/10129/20180829/Raw/EyeData/txt/10129.20180829.1.data.tsv
 files     <- Sys.glob(filesFrom)
 splitfile <- strsplit(basename(files),'\\.')
 splitfile <- as.data.frame(t(sapply(splitfile,rbind)))[,-5][,1:3]
